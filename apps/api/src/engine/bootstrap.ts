...
import {HomeAssistant} from "./core/core";
...
const ERROR_LOG_FILENAME = "home-assistant.log"

// hass.data key for logging information.
const DATA_LOGGING = "logging"

const LOG_SLOW_STARTUP_INTERVAL = 60

const STAGE_1_TIMEOUT = 120
const STAGE_2_TIMEOUT = 300
const WRAP_UP_TIMEOUT = 300
const COOLDOWN_TIME = 60

const MAX_LOAD_CONCURRENTLY = 6

const DEBUGGER_INTEGRATIONS = ["debugpy"]
const CORE_INTEGRATIONS = ["homeassistant", "persistent_notification"]
const LOGGING_INTEGRATIONS = [
    // Set log levels
    "logger",
    // Error logging
    "system_log",
...
export async function async_setup_hass(
    ...
    if (!safe_mode) {
        ...
        try {
            ...
            if (success) {
                ...
                const config = await async_from_config_dict(config_dict, hass)
                basic_setup_success = !!config;
            }
        }
        finally {
            clear_secret_cache()
    ...
    if (!config_dict) {
        ...
        !hass.data.get(DATA_SETUP, {})["frontend"]
        &&  !hass.config.components.has("frontend")
    ) {
 ...
 * Open the UI.
 */
export function open_hass_ui(hass: core.HomeAssistant): None {
...
export async function async_from_config_dict(
    ...
    const core_domain_components_init_results = await Promise.all(CORE_INTEGRATIONS.map(domain => async_setup_component(hass, domain, config)))
    if (core_domain_components_init_results.some(res => !res)) {
 ...
 */
export async function async_mount_local_lib_path(config_dir: str): str {
 ...
 */
export function _get_domains(hass: core.HomeAssistant, config: Dict[str, Any]): Set[str] {
