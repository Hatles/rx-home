...
export const BENCHMARKS: Dict<Callable>  = {}
...
async def run_benchmark(bench):
    ...
    print`Benchmark ${bench.__name__} done in ${runtime}s`)
 ...
 */
export function benchmark(func: CALLABLE_T): CALLABLE_T {
...
async def time_changed_helper(hass):
    ...
    event_data = {ATTR_NOW: datetime(2017, 10, 10, 15, 0, 0, tzinfo=UTC)}
...
async def state_changed_helper(hass):
    ...
    for(const idx of range(1000)) {
        hass.helpers.event.async_track_state_change(
           `${entity_id}${idx}", listener, "off", "on`
        )
    }
    event_data = {
        "entity_id":`${entity_id}0`,
        "old_state": State(entity_id, "off"),
...
async def state_changed_event_helper(hass):
    ...
    hass.helpers.event.async_track_state_change_event(
        `${entity_id}${idx}` for idx in range(1000)], listener
    )
...
async def _logbook_filtering(hass, last_changed, last_updated):
    ...
    event = _create_state_changed_event_from_old_new(
        entity_id, utcnow(), old_state, new_state
    )
...
async def filtering_entity_id(hass):
    ...
    size = entity_ids.length

    start = timer()

    for(const i of range(10 ** 5)) {
...
async def json_serialize_states(hass):
    ...
    states = [
        State("light.kitchen", "on", {"friendly_name": "Kitchen Lights"})
        for _ in range(10 ** 6)
    ]
...
def _create_state_changed_event_from_old_new(
    ...
    row.domain = entity_id and split_entity_id(entity_id)[0]
    row.context_id = null
    row.context_user_id = null
