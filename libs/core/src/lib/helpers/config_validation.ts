...
export const byte = vol.All(vol.Coerce(int), vol.Range(min=0, max=255))
export const small_float = vol.All(vol.Coerce(float), vol.Range(min=0, max=1))
export const positive_int = vol.All(vol.Coerce(int), vol.Range(min=0))
export const positive_float = vol.All(vol.Coerce(float), vol.Range(min=0))
export const latitude = vol.All(
    vol.Coerce(float), vol.Range(min=-90, max=90), msg="invalid latitude"
)
export const longitude = vol.All(
    vol.Coerce(float), vol.Range(min=-180, max=180), msg="invalid longitude"
)
export const gps = vol.ExactSequence([latitude, longitude])
export const sun_event = vol.All(vol.Lower, vol.Any(SUN_EVENT_SUNSET, SUN_EVENT_SUNRISE))
export const port = vol.All(vol.Coerce(int), vol.Range(min=1, max=65535))
...
export function path(value: any): string {
    if (!value instanceof string) {
        throw new vol.Invalid("Expected a string")
    }

    if (sanitize_path(value) !== value) {
        throw new vol.Invalid("Invalid path")
    }
...
export function has_at_least_one_key(*keys: string): Callable {
    ...
    validate(obj: Dict): Dict {
        if (!obj instanceof dict) {
            throw new vol.Invalid("expected dictionary")
        }
        ...
        throw new vol.Invalid("must contain at least one of {}.".format(", ".join(keys)))
    }
...
export function has_at_most_one_key(*keys: string): Callable[[Dict], Dict] {
    ...
    validate(obj: Dict): Dict {
        ...
        if (set(keys.length & set(obj)) > 1) {
            throw new vol.Invalid("must contain at most one of {}.".format(", ".join(keys)))
        }
        return obj
    }
...
export function boolean(value: any): boolean {
    ...
    throw new vol.Invalid`invalid boolean value ${value}`)
}
...
export function whitespace(value: any): string {
    ...
    throw new vol.Invalid`contains non-whitespace: ${value}`)
}
...
export function isdevice(value: any): string {
    try {
        ...
        throw new vol.Invalid`No device at ${value} found`) from err
        }
        else {
            throw e;
        }
...
export function matches_regex(regex: string): Callable[[Any], string] {
    ...
    validator(value: any): string {
        if (!value instanceof string) {
            throw new vol.Invalid`not a string value: ${value}`)
        }

        if (!compiled.match(value) {
            throw new vol.Invalid(
...
export function is_regex(value: any): Pattern<Any> {
    try {
        ...
        throw new vol.Invalid`value ${value} is not a valid regular expression`) from err
        }
        else {
            throw e;
        }
...
export function isfile(value: any): string {
    if (!value) {
        throw new vol.Invalid("null is not file")
    }
    file_in = os.path.expanduser(str(value))

    if (!os.path.isfile(file_in) {
        throw new vol.Invalid("not a file")
    }
    if (!os.access(file_in, os.R_OK) {
        throw new vol.Invalid("file not readable")
    }
    return file_in
}
...
export function isdir(value: any): string {
    if (!value) {
        throw new vol.Invalid("not a directory")
    }
    ...
    if (!os.access(dir_in, os.R_OK) {
        throw new vol.Invalid("directory not readable")
    }
    return dir_in
}
...
export function entity_id(value: any): string {
    ...
    throw new vol.Invalid`Entity ID ${value} is an invalid entity id`)
}
...
export function entity_ids(value: Union<string, List>): string[] {
    if (!value) {
        throw new vol.Invalid("Entity IDs can not be null")
    }
    if (value instanceof string) {
...
export const comp_entity_ids = vol.Any(
...
export function entity_domain(domain: Union<string, string>]): Callable[[Any], string[] {
    ...
    validate(value: string): string {
        ...
        if (validated.length !== 1) {
            throw new vol.Invalid`Expected exactly 1 entity, got ${validated.length}`)
        }
        return validated[0]
    }
 ...
 */
export function enum(enumClass: Type<Enum>): vol.All {
...
export function icon(value: any): string {
    ...
    throw new vol.Invalid('Icons should be specified in the form "prefix:name"')
}

export const time_period_dict = vol.All(
    dict,
    vol.Schema(
        {
...
export function time(value: any): time_sys {
    ...
    try {
        ...
        if (e instanceof TypeError as err) {
        throw new vol.Invalid("Not a parseable type") from err
        }
        else {
            throw e;
        }
    ...
    if (!time_val) {
        throw new vol.Invalid`Invalid time specified: ${value}`)
    }
...
export function date(value: any): date_sys {
    ...
    if (!date_val) {
        throw new vol.Invalid("Could not parse date")
    }
...
export function time_period_str(value: string): timedelta {
    if (value instanceof number) {
        throw new vol.Invalid("Make sure you wrap time values in quotes")
    }
    if (!value instanceof string) {
        throw new vol.Invalid(TIME_PERIOD_ERROR.format(value))
    }
    ...
    try {
        ...
        throw new vol.Invalid(TIME_PERIOD_ERROR.format(value)) from err
        }
        else {
            throw e;
        }
...
export function time_period_seconds(value: Union<number, string>): timedelta {
    try {
        ...
        throw new vol.Invalid`Expected seconds, got ${value}`) from err
        }
        else {
            throw e;
        }
...
export const time_period = vol.Any(time_period_str, time_period_seconds, timedelta, time_period_dict)
...
export function positive_timedelta(value: timedelta): timedelta {
    if (value < timedelta(0) {
        throw new vol.Invalid("Time period should be positive")
    }
    return value
}

export const positive_time_period_dict = vol.All(time_period_dict, positive_timedelta)
export const positive_time_period = vol.All(time_period, positive_timedelta)
...
export function service(value: any): string {
    ...
    throw new vol.Invalid`Service ${value} does not match format <domain>.<name>`)
}
...
export function slug(value: any): string {
    if (!value) {
        throw new vol.Invalid("Slug should not be null")
    }
    strvalue = string(value)
    ...
    throw new vol.Invalid`invalid slug ${value} (try ${slg})`)
}
...
export function slugify(value: any): string {
    ...
    throw new vol.Invalid`Unable to slugify ${value}`)
}
...
export function string(value: any): string {
    if (!value) {
        throw new vol.Invalid("string !value")
    }
    ...
    if (value instanceof template_helper.ResultWrapper) {
        ...
        throw new vol.Invalid("value should be a string")
...
export function strwith_no_html(value: any): string {
    ...
    if (regex.search(value) {
        throw new vol.Invalid("the string should not contain HTML")
    }
    return string(value)
}
...
export function temperature_unit(value: any): string {
    ...
    throw new vol.Invalid("invalid temperature unit (expected C or F)")
}

export const unit_system = vol.All(
...
export function template(value: Optional<Any>): template_helper.Template {
    ...
    if (!value) {
        throw new vol.Invalid("template !value")
    }
    if value instanceof (list, dict, template_helper.Template):
        throw new vol.Invalid("template value should be a string")
    ...
    try {
        ...
        throw new vol.Invalid`invalid template (${ex})`) from ex
        }
        else {
            throw e;
        }
...
export function dynamic_template(value: Optional<Any>): template_helper.Template {
    ...
    if not template_helper.is_template_string(str(value)):
        throw new vol.Invalid("template value does not contain a dynmamic template")
...
export const positive_time_period_template = vol.Any(
...
export function datetime(value: any): datetime_sys {
    ...
    if (!date_val) {
        throw new vol.Invalid`Invalid datetime specified: ${value}`)
    }
...
export const weekdays = vol.All(ensure_list, [vol.In(WEEKDAYS)])
...
export function socket_timeout(value: Optional<Any>): object {
    ...
    try {
        ...
        throw new vol.Invalid("Invalid socket timeout value. number > 0.0 required.")
    }
    catch (e) {
        if (e instanceof Exception as err) {
        throw new vol.Invalid`Invalid socket timeout: ${err}`)
        }
        else {
            throw e;
        }
...
export function url(value: any): string {
    ...
    throw new vol.Invalid("invalid url")
}
...
def x10_address(value: string) -> string:
    ...
    if (!regex.match(value) {
        throw new vol.Invalid("Invalid X10 Address")
    }
...
def uuid4_hex(value: any) -> string:
    ...
    try {
        ...
        throw new vol.Invalid("Invalid Version4 UUID", error_message=str(error))
        }
        else {
            throw e;
        }
    ...
    if (result.hex !== value.lower() {
        ...
        throw new vol.Invalid("Invalid Version4 UUID")
    }
...
export class multi_select {
    ...
    __call__(selected: list): list {
        if (!selected instanceof list) {
            throw new vol.Invalid("Not a list")
        }

        for(const value of selected) {
            if (value !in this.options) {
                throw new vol.Invalid`${value} is not a valid option`)
            }
        }
...
export function key_value_schemas(
    key: string, value_schemas: Dict<vol.Schema>
...
export function key_dependency(
    ...
    validator(value: Dict<Hashable, any>): Dict<Hashable, any> {
        if (!value instanceof dict) {
            throw new vol.Invalid("key dependencies require a dict")
        }
...
// Schemas
export const PLATFORM_SCHEMA = vol.Schema(
    {
...
export const PLATFORM_SCHEMA_BASE = PLATFORM_SCHEMA.extend({}, extra=vol.ALLOW_EXTRA)
...
export function make_entity_service_schema(
    schema: dict, *, extra: number = vol.PREVENT_EXTRA
): vol.All {
    return vol.All(
        vol.Schema(
            {
...
export const SCRIPT_VARIABLES_SCHEMA = vol.All(
...
export const SCRIPT_SCHEMA = vol.All(ensure_list, [script_action])

export const EVENT_SCHEMA = vol.Schema(
    {
...
export const SERVICE_SCHEMA = vol.All(
    vol.Schema(
        {
            vol.Optional(CONF_ALIAS): string,
...
export const NUMERIC_STATE_THRESHOLD_SCHEMA = vol.Any(
    vol.Coerce(float), vol.All(str, entity_domain("input_number"))
)

export const NUMERIC_STATE_CONDITION_SCHEMA = vol.All(
    vol.Schema(
        {
...
export const STATE_CONDITION_BASE_SCHEMA = {
    ...
    vol.Optional("from"): string,
}

export const STATE_CONDITION_STATE_SCHEMA = vol.Schema(
    {
...
export const STATE_CONDITION_ATTRIBUTE_SCHEMA = vol.Schema(
    {
        **STATE_CONDITION_BASE_SCHEMA,
...
export const SUN_CONDITION_SCHEMA = vol.All(
    vol.Schema(
        {
...
export const TEMPLATE_CONDITION_SCHEMA = vol.Schema(
    {
...
export const TIME_CONDITION_SCHEMA = vol.All(
    vol.Schema(
        {
...
export const ZONE_CONDITION_SCHEMA = vol.Schema(
    {
...
export const AND_CONDITION_SCHEMA = vol.Schema(
    {
...
export const OR_CONDITION_SCHEMA = vol.Schema(
    {
...
export const NOT_CONDITION_SCHEMA = vol.Schema(
    {
...
export const DEVICE_CONDITION_BASE_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_CONDITION): "device",
        vol.Required(CONF_DEVICE_ID): string,
        vol.Required(CONF_DOMAIN): string,
    }
)

export const DEVICE_CONDITION_SCHEMA = DEVICE_CONDITION_BASE_SCHEMA.extend({}, extra=vol.ALLOW_EXTRA)

export const CONDITION_SCHEMA: vol.Schema  = vol.Schema(
    vol.Any(
        key_value_schemas(
            CONF_CONDITION,
            {
...
export const TRIGGER_SCHEMA = vol.All(
    ensure_list, [vol.Schema({vol.Required(CONF_PLATFORM): string}, extra=vol.ALLOW_EXTRA)]
)

export const _SCRIPT_DELAY_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
...
export const _SCRIPT_WAIT_TEMPLATE_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
...
export const DEVICE_ACTION_BASE_SCHEMA = vol.Schema(
    {vol.Required(CONF_DEVICE_ID): string, vol.Required(CONF_DOMAIN): string}
)

export const DEVICE_ACTION_SCHEMA = DEVICE_ACTION_BASE_SCHEMA.extend({}, extra=vol.ALLOW_EXTRA)

export const _SCRIPT_SCENE_SCHEMA = vol.Schema({vol.Required(CONF_SCENE): entity_domain("scene")})

export const _SCRIPT_REPEAT_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
...
export const _SCRIPT_CHOOSE_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
...
export const _SCRIPT_WAIT_FOR_TRIGGER_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
...
export const _SCRIPT_SET_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_ALIAS): string,
