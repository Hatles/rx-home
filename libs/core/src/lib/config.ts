...
export const PACKAGES_CONFIG_SCHEMA = cv.schema_with_slug_keys(  // Package names are slugs
    vol.Schema({cv.string: vol.Any(dict, list, null)})  // Component config
)

export const CUSTOMIZE_DICT_SCHEMA = vol.Schema(
    {
...
export const CUSTOMIZE_CONFIG_SCHEMA = vol.Schema(
    {
...
export function _identify_config_schema(module: ModuleType): Optional<string> {
    if (!module.CONFIG_SCHEMA instanceof vol.Schema) {
        return null
    }

    schema = module.CONFIG_SCHEMA.schema  // type: ignore

    if (schema instanceof vol.All) {
    ...
    if (hasattr(key, "default") and !
        key.default instanceof vol.schema_builder.Undefined
     {
