 ...
 */
export function is_loopback(address: (IPv4Address | IPv6Address)): boolean {
 ...
 */
export function is_private(address: (IPv4Address | IPv6Address)): boolean {
 ...
 */
export function is_link_local(address: (IPv4Address | IPv6Address)): boolean {
 ...
 */
export function is_local(address: (IPv4Address | IPv6Address)): boolean {
    return is_loopback(address) || is_private(address)
}
...
export function normalize_url(address: string): string {
    const url = new URL(address.rstrip("/"))
    if (url.is_default_port()) {
        return url.with_port(null).toString()
    }
    return url.toString()
}
