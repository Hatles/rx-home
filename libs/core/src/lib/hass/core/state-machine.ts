...
import {EVENT_STATE_CHANGED} from "../const";
...
export class StateMachine {
    ...
    */
     constructor (bus: EventBus) {
        ...
        this._reservations = new Set<string>()
        this._bus = bus
    }
    ...
    */
     entity_ids (domain_filter: string | string[] = null): string[] {
        if (!domain_filter) {
    ...
    */
     remove (entity_id: string, context: Context = null): boolean {
        ...
        this._bus.fire(
            EVENT_STATE_CHANGED,
            {"entity_id": entity_id, "old_state": old_state, "new_state": null},
            EventOrigin.local,
            context,
        )
    ...
    */
     available (entity_id: string): boolean {
        entity_id = entity_id.toLowerCase()
    ...
    */
     set (
        ...
        if (!old_state) {
            ...
            same_attr = old_state.attributes === attributes // todo : check each properties
