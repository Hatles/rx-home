...
export const CONFIG_SCHEMA = MULTI_FACTOR_AUTH_MODULE_SCHEMA.extend(
    {
        Joi.Optional(CONF_INCLUDE): Joi.All(cv.ensure_list, [cv.string]),
        Joi.Optional(CONF_EXCLUDE): Joi.All(cv.ensure_list, [cv.string]),
        Joi.Optional(CONF_MESSAGE, default="{} is your Home Assistant login code"): string,
    },
...
export class NotifyAuthModule extends MultiFactorAuthModule {
    ...
    input_schema(): Joi.Schema {
        return Joi.Schema({INPUT_FIELD_CODE: string})
    }
...
export class NotifySetupFlow extends SetupFlow {
    ...
    async async_step_init(
        ...
        schema["notify_service"] = Joi.In(this._available_notify_services)
        schema["target"] = Joi.Optional(str)
