...
export const REQUIREMENTS = ["keyring===21.2.0", "keyrings.alt===3.4.0"]
...
def run(args):
    ...
    if (args.action === "info") {
        ...
        print`Active keyring  : ${keyr.__module__}`)
        config_name = os.path.join(platform.config_root(), "keyringrc.cfg")
        print`Config location : ${config_name}`)
        print`Data location   : ${platform.data_root()}\n`)
    }
    else if *(!args.name) {
        parser.print_help()
        return 1
    ...
    if (args.action === "set") {
        entered_secret = getpass.getpass`Please enter the secret for ${args.name}: `)
        keyring.set_password(_SECRET_NAMESPACE, args.name, entered_secret)
        print`Secret ${args.name} set successfully`)
    }
        ...
        try {
            ...
            if (e instanceof keyring.errors.PasswordDeleteError) {
            print`Secret ${args.name} not found`)
            }
            else {
                throw e;
